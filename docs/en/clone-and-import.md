...
`git clone https://github.com/ByteLegendQuest/java-fix-method-passing-value`

A project directory named `java-fix-method-passing-value` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-method-passing-value` directory which is created just now.
